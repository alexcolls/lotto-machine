...
async function uploadV2({ files, cacheName, env, totalNFTs, storage, retainAuthority, mutable, nftStorageKey, ipfsCredentials, awsS3Bucket, batchSize, price, treasuryWallet, splToken, gatekeeper, goLiveDate, endSettings, whitelistMintSettings, hiddenSettings, uuid, walletKeyPair, anchorProgram, arweaveJwk, rateLimit, }) {
    var _a, _b;
            ...
            await Promise.all((0, various_1.chunks)(Array.from(Array(SIZE).keys()), batchSize || 50).map(async (allIndexesInSlice) => {
                for (let i = 0; i < allIndexesInSlice.length; i++) {
                    ...
                    if ('animation_url' in manifest) {
                        const animationPath = path_1.default.join(dirname, `${manifest.animation_url}`);
                        if (fs_1.default.existsSync(animationPath) &&
                            fs_1.default.lstatSync(animationPath).isFile()) {
                    ...
                    try {
                        ...
                        if (animation) {
                            loglevel_1.default.error(`Error uploading files ${manifest.image} + ${manifest.animation_url}`, err);
                        }
                        else {
                            loglevel_1.default.error(`Error uploading file ${manifest.image}`, err);
                        }
                        i--;
                    }
                }
 ...
 */
async function writeIndices({ anchorProgram, cacheContent, cacheName, env, candyMachine, walletKeyPair, uploadSuccessful, rateLimit, }) {
    ...
    try {
            ...
            if (onChain.length != indexes.length) {
                promiseArray.push(() => {
                    ...
                    return anchorProgram.rpc
                        .addConfigLines(ind, indexes.map(i => ({
        ...
        promiseArray = promiseArray.map((p, i) => {
            return async () => {
                await (0, various_1.sleep)(1000 * Math.floor(i / rateLimit));
                await p();
            };
        });
