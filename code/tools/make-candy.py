...
input("\n> CREATE CANDY? Enter to go or CTRL+C to exit -> ")

net = "devnet" # mainnet-beta or devnet
owner_wallet = "virus" # insert wallet name
...
def LuckyCat():
    ...
    w = os.getcwd()+"\tools\keys\""+owner_wallet+".json"
    print("\nWallet keypair path: ",w)
    os.system("solana config set -k"+w)
    src = 'assets/'
    try:
        shutil.rmtree(src)
        shutil.rmtree('.cache')
    ...
    if( cmnd == 'y' or cmnd == 'yes' ):
        ...
        os.system("ts-node ./tools/lib/metaplex/js/packages/cli/src/candy-machine-v2-cli.ts upload -e "+net+" -k "+w+" -cp ./config.json -c cache "+src)
